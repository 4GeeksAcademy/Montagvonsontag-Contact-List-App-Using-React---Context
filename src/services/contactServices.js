// Includes all functionalities related to the services 
...
contactServices.deleteContact = async(slug, id)=>{
    try {
        ...
        if (!resp.ok) throw new Error('error when deleting')
...
contactServices.updateContact = async (slug, id, formData) => {
    try {
        ...
        if (!resp.ok) throw new Error('error when updating')

        //updated version of agenda:
        return contactServices.getOneAgenda(slug) 
    } catch (error) {
        console.log(error);
    }
